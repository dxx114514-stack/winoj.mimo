...
封禁用户的 Access Token 和 Refresh Token 均会被拒绝。
...
提交代码时自动调用本地 Ollama（默认 `localhost:11434`，模型 `qwen3:1.7b`）进行安全审查。发现恶意代码将自动封禁用户。代码少于 50 字符跳过审查。源代码上限 50000 字符。
...
无需认证，需携带 HttpOnly Cookie 中的 refresh_token。封禁用户的 refresh token 会被拒绝并删除。
...
```json
[
  { "id": 1, "name": "动态规划", "color": "#6366f1", "created_at": "2024-01-01 00:00:00" },
  { "id": 2, "name": "图论", "color": "#10b981", "created_at": "2024-01-01 00:00:00" }
]
```
...
需认证，有速率限制。源代码上限 50000 字符。自动触发 AI 安全审查。

**安全审查失败时：** 返回 403，用户被自动封禁。
...
需认证。可更新 `nickname`、`signature`（1000字限制）、`bio`（Markdown）。
...
需认证。multipart/form-data，字段名 `file`。最大 10MB，单用户最大 2GB。
...
| 题目详情 | `/pages/problem.html?id=X` | Markdown 题面、题解 |
| 提交代码 | `/pages/submit.html?id=X` | 代码编辑器、提交 |
| 提交记录 | `/pages/submissions.html` | 提交历史列表 |
| 提交详情 | `/pages/submission.html?id=X` | 评测详情、错误说明、重测 |
...
| 个人资料 | `/pages/profile.html` | 签名、简介、编辑 |
| Rating 排行 | `/pages/rating.html` | Rating 排行榜 |
| 图床 | `/pages/upload.html` | 文件上传、URL 复制 |
